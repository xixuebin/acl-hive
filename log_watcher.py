...
if __name__ == '__main__':
    ...
    def sendmail(message):
        try:
            message_header = """From: ACL-Hive-Monitor <info@dianping.com>
Subject: ACL-Hive Error

error occurred: see more as follows:\n
"""
            smtpObj = smtplib.SMTP(smtp_host)
            smtpObj.login(smtp_user, smtp_passwd)
            smtpObj.sendmail(sender, receivers, message_header+message)         
    ...
    def callback(filename, lines):
        for line in lines:
            if line.find('[ERROR] error task node:')!=-1:
                sendmail(line)

    l = LogWatcher(log_location, callback)
    l.loop()
